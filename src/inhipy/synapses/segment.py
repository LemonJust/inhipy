...
def load_image(img_file, plot=True):
    """
...
class SegmentationTask:
    ...
    def __init__(self, 
        ...
        """
        Args:
            image_files: a list of image files to segment
    ...
    def from_config_dict(cls, d):
        ...
        for tp in d['data']['image_files']:
            image_files[tp] = basedir / d['data']['image_files'][tp]

        # create output folder
        output_files = d['output']
        step = d.get('step', 'synapse_candidates')
        name = d.get('name', generate_slug(2)) # generate random name if not provided
        output_folder = Path(output_files['basedir']) / step / name

        return cls(image_files, model, parameters, output_files, output_folder)
    ...
    def segment_image(self, img_file, tp_name, verbose=True):
        """
        Segments the image and saves the results to the output directory.
        """
        # load the image
        img = load_image(img_file, plot=False)
    ...
    def run(self, verbose=True):
        ...
        for tp_name, img_file in self.image_files.items():
            ...
            labels, details = self.segment_image(img_file, tp_name, verbose=verbose)
